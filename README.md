...
dev环境下，我们的目的是更直观、更方便的读写配置文件；test和prod环境下，出于安全运维的考虑，一部分重要的或者全部的应用配置一般都会有配置中心统一管理
...
我的观点是尽量少拆分甚至不拆分。原因是现在很难会出现所谓的一个应用（也可以理解为一个jar）由多个团队（或者许多许多人）开发维护的场景，所以一般就不会有同环境下存在很多个配置文件（配置集）的情况。
由于很少出现，所以将原来1~2个配置文件组成的全部配置拆分成若干个分工明确的小配置文件，我认为反而会增大日常的开发维护成本，增大编码的复杂度，同样的，如果没有特别特别细分的配置权限管理，分成许多个小的子集也是不合理的。

那么有人会质疑，之所以做拆分，是因为想要实现修改配置可以实时地动态更新，拆分之后会减轻应用更新配置时的资源消耗。

其实，以基于Nacos的微服务配置动态更新为例，如果我们在Nacos端修改了某一个配置key，对应的微服务应用其实只会更新这一条配置，以及依赖它的某几个Bean（如果有的话），所以配置的更新并非是文件维度的，而是key维度，所以谈资源消耗就没什么必要了。下图可以简单看出其中的端倪。
